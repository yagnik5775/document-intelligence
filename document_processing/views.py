        ...
        if form.is_valid():
            try:
                ...
                if not document.pdf_file.name.lower().endswith('.pdf'):
                    ...
                    return redirect("upload")  # or wherever your upload form lives
                ...
                chunk_metadata, index = process_new_document(full_path, document.id)

                # clean metadata
                clean_chunks = [{k: v.replace('\x00', '') if isinstance(v, str) else v
                                 for k, v in chunk.items()} for chunk in chunk_metadata]

                document.chunk_metadata = json.dumps(clean_chunks)
                document.faiss_index = index
                document.save()

                messages.success(request, "Document uploaded successfully")
                return redirect("dashboard")   # <--- full reload to dashboard
...
def allDocuments(request):
    ...
    # ---- Only required change starts here ----
    ...
    # ---- Only required change ends here ----
...
def analyzePdf(request, docId):
    ...
    try:
        ...
        print(f"Chunk metadata: {chunk_metadata[:5]}")  
        ...
        if request.headers.get("accept") == "application/json":
            ...
            messages.success(request, f"Processed {len(chunks)} chunks successfully")
            return redirect("dashboard")
        # ---- Only required change ends here ----
        ...
        if request.headers.get("accept") == "application/json":
            ...
            from django.contrib import messages
            from django.shortcuts import redirect
            messages.error(request, f"Analyze failed: {str(e)}")
            return redirect("dashboard")
